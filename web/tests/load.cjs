...
exports.load = (relative, mocks = {}, globals = {}) => {
  ...
  vm.runInNewContext(code, {
    module, exports: module.exports, require: (name) => { if (name in mocks) return mocks[name]; if (name === './addonChanges') return exports.load('lib/addonChanges.ts'); if (name === './stalker') return exports.load('lib/stalker.ts', { './http': mocks['./http'] ?? {} }); if (name === './iptvSession') return exports.load('lib/iptvSession.ts'); if (name.startsWith('node:') || ['ipaddr.js', 'undici'].includes(name)) return require(name); throw new Error(`Unmocked dependency ${name}`); },

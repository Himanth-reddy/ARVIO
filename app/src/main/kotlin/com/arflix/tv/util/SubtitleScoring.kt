...
private val PREFIX_BRACKET_RE = Regex("^\\[[^]]+]")
private val SEP_RE = Regex("[.\\-_\\s]+")

private fun tokenWeight(token: String): Int = when {
    token.matches(DIGITS_RE) -> 0               // pure numbers: noise
...
fun weightedSubtitleScore(streamSource: String, subtitleId: String): Int {
    ...
    val cleanId = subtitleId.replace(PREFIX_BRACKET_RE, "").trim()
    ...
    val streamTokens = streamBody.lowercase().split(SEP_RE).filter { it.length > 1 }
    val subTokenSet  = subBody.lowercase().split(SEP_RE).filter { it.length > 1 }.toSet()

...
class SportsDestinationTest {
    ...
    @Test
    fun applyingItTwiceLeavesTheSingleMatchGuideRowWhereItWas() {
        val once = tree(
            LiveCategory(SPORTS_CHANNEL_CATEGORY, "Sports · Global", 41, CategoryIcon.Sport),
        ).withSportsDestination()

        assertThat(once.withSportsDestination().top).isEqualTo(once.top)
    }

    @Test
    fun withoutSportsChannelsTheGuideStaysATopLevelRowSoAddonSportIsReachable() {
